...
@WebServlet(urlPatterns = "/RollUp.ca")
public class RollUp extends HttpServlet{
...
protected void doPost(HttpServletRequest request, HttpServletResponse response)
	 ...
	 String date = request.getParameter("dateofenroll");
	 WelcomeService welcomeservice = new WelcomeService();
	 GeneralServicewithId generalservice=new GeneralServicewithId();
	 List <EducationSystemFact> educationList= welcomeservice.rollUp(date);
	 for(int i =0;i<educationList.size();i++) {
		...
		request.getRequestDispatcher("/WEB-INF/views/RollUp.jsp").forward(request, response);
